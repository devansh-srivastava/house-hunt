...
async function saveQuoteStatus(quoteId, status) {
  if (!supabase) {
    const q = DEMO.quotes.find(q => q.id === quoteId);
    if (q) q.status = q.status === status ? null : status;
    return;
  }
  const { data: existing } = await supabase.from('broker_quotes').select('status').eq('id', quoteId).single();
...
function attachQuoteEvents(quotes) {
  document.querySelectorAll('.status-btn').forEach(btn => {
    btn.addEventListener('click', async (e) => {
      ...
      await saveQuoteStatus(quoteId, status);
      // Re-render current config quotes
